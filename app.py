...
def validate_outline(outline: str, previous_outlines: List[str] = None) -> bool:
    ...
    for prev_outline in previous_outlines:
        similarity = SequenceMatcher(None, outline.lower(), prev_outline.lower()).ratio()
...
class OutlineReviewer:
    ...
    def review_outline(self, outline: str, chapter_num: int, premise: str) -> Dict:
        ...
        review["outline_unique"] = validate_outline(outline, previous_outlines)
        if not review["outline_unique"]:
            review["issues"].append("Outline is too similar to a previous chapter")
        
        # Check premise alignment using basic keyword matching
        premise_keywords = set(premise.lower().split())
        outline_keywords = set(outline.lower().split())
        keyword_overlap = len(premise_keywords.intersection(outline_keywords)) / len(premise_keywords)
        review["premise_aligned"] = keyword_overlap >= 0.3
        if not review["premise_aligned"]:
            review["issues"].append("Outline may not align well with the original premise")
...
Create a detailed outline for Chapter {chapter_num}. Include:
...
Make sure the chapter's content is unique compared to previous chapters.
